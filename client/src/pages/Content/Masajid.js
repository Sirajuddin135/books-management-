...
const Masajid = () => {
    ...
    useEffect(() => {
        ...
        const fetchOptions = async (parameters) => {
            await axios.post(`http://localhost:4000/api/searchOptions`, parameters)
                .then(response => {
                    setCities(response.data.options);
                }).catch((err) => {
                    console.error(err);
                })
        }
        ...
        if (selectedCountry && selectedState && selectedCity) {
            parameters = { queryType: "state", query: selectedState, keyword: "area" };
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
                ...
                {selectedOption === 'masjid' && (
                    <label>
                        ...
                        <select value={selectedMasjid} onChange={handleMasjidChange}>
